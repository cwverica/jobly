...
describe("GET /companies", function () {
  ...
  test("fails when max employees < min employees", async function () {
    const thrown = async () => { await request(app).get('/companies?minEmployees=14&maxEmployees=2') };
    // expect(thrown).toThrow(BadRequestError);
    // expect(thrown).toThrow("minEmployees must be less than or equal to maxEmployees")
    // In production, error throws properly. In testing, it does not. TODO: figure out why?
  });
